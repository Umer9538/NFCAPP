...
import { GRAY, STATUS, SEMANTIC } from '@/constants/colors';
import { spacing, borderRadius, shadows, typography } from '@/theme/theme';
import { ThemeContext } from '@/theme/ThemeProvider';
import { defaultTheme, PrimaryPalette } from '@/theme/colors';
...
export function Button({
  ...
  // Get dynamic theme colors (fall back to default if context not available)
  ...
  // Get variant styles with dynamic colors
  const variantStyle = getVariantStyle(variant, primary);
  const variantTextStyle = getVariantTextStyle(variant, primary);
      ...
      <View style={styles.content}>
        {loading ? (
          <ActivityIndicator
            ...
            color={getSpinnerColor(variant, primary)}
...
/**
 * Get variant styles with dynamic primary colors
 */
function getVariantStyle(variant: ButtonVariant, primary: PrimaryPalette) {
  switch (variant) {
    case 'primary':
      return {
        default: {
...
/**
 * Get variant text styles with dynamic primary colors
 */
function getVariantTextStyle(variant: ButtonVariant, primary: PrimaryPalette): TextStyle {
  switch (variant) {
    case 'primary':
    case 'danger':
      return { color: '#ffffff' };
...
/**
 * Get spinner color with dynamic primary colors
 */
function getSpinnerColor(variant: ButtonVariant, primary: PrimaryPalette): string {
  switch (variant) {
    case 'primary':
    case 'danger':
      return '#ffffff';
