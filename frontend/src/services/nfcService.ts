/**
 * NFC Service
 * Handle NFC operations and scanning for real NFC bracelets
 * Supports iOS CoreNFC and Android NFC
 */
...
class NFCService {
  ...
  /**
   * Start NFC scanning - optimized for real NFC bracelets
   * Supports both NDEF tags and raw NFC tags (just reading tag ID)
   */
  async startScanning(
    ...
    if (this.isMockMode) {
      ...
      if (onError) {
        onError(new Error('NFC scanning requires a development build. Please build the app with EAS Build to test NFC functionality with real bracelets.'));
      }
      return;
    }
    ...
    try {
      ...
      if (Platform.OS === 'ios') {
        // iOS: Use CoreNFC with proper session handling
        await this.scanIOS(onTagDiscovered, onError);
      } else {
        // Android: Use tag discovery
        await this.scanAndroid(onTagDiscovered, onError);
      }
    } catch (error: any) {
  ...
  /**
   * iOS-specific NFC scanning using CoreNFC
   */
  private async scanIOS(
    ...
    try {
      // Try NDEF first (most common for smart tags)
      await NfcManager.requestTechnology(NfcTech.Ndef, {
        alertMessage: 'Hold your iPhone near the NFC bracelet',
      });

      const tag = await NfcManager.getTag();
      console.log('📱 iOS NFC Tag discovered:', JSON.stringify(tag, null, 2));
      ...
      if (tag) {
        ...
        console.log('✅ Extracted NFC Tag:', nfcTag);
        onTagDiscovered(nfcTag);
      }
    } catch (error: any) {
      // If NDEF fails, try MifareIOS for raw tag reading
      if (error.message?.includes('cancelled') || error.message?.includes('invalidated')) {
        throw error;
      }

      console.log('NDEF failed, trying MifareIOS...');
      try {
  ...
  /**
   * Android-specific NFC scanning
   */
  private async scanAndroid(
    onTagDiscovered: (tag: NFCTag) => void,
    onError?: (error: Error) => void
  ): Promise<void> {
    try {
      // Try multiple technologies for better compatibility
      const techTypes = [NfcTech.Ndef, NfcTech.NfcA, NfcTech.NfcB, NfcTech.NfcF, NfcTech.NfcV, NfcTech.IsoDep];
      ...
      if (tag) {
        console.log('📱 Android NFC Tag discovered:', JSON.stringify(tag, null, 2));
        const tagId = this.extractTagId(tag);
        const nfcTag: NFCTag = {
          id: tagId,
          type: this.getTagType(tag),
   ...
   * Handles different tag types and ID formats
   */
  private extractTagId(tag: any): string {
    if (!tag) return '';

    // Try different ID properties
    let rawId = tag.id || tag.identifier || tag.uid || tag.serialNumber || '';

    // If ID is an array of bytes, convert to hex string
    if (Array.isArray(rawId)) {
      rawId = rawId.map((byte: number) => byte.toString(16).padStart(2, '0')).join('').toUpperCase();
    }

    // If ID is already a string, clean it up
    if (typeof rawId === 'string') {
      // Remove any colons or spaces and uppercase
      rawId = rawId.replace(/[:\s-]/g, '').toUpperCase();
    }

    console.log('🏷️ Extracted Tag ID:', rawId);
    return rawId;
  }
  ...
  private getTagType(tag: any): string {
    ...
    // Check tech types for Android
    if (tag.techTypes && Array.isArray(tag.techTypes)) {
      if (tag.techTypes.includes('android.nfc.tech.Ndef')) return 'NDEF';
      if (tag.techTypes.includes('android.nfc.tech.NfcA')) return 'NFC-A (ISO 14443-3A)';
      if (tag.techTypes.includes('android.nfc.tech.NfcB')) return 'NFC-B (ISO 14443-3B)';
      if (tag.techTypes.includes('android.nfc.tech.NfcF')) return 'NFC-F (FeliCa)';
      if (tag.techTypes.includes('android.nfc.tech.NfcV')) return 'NFC-V (ISO 15693)';
      if (tag.techTypes.includes('android.nfc.tech.IsoDep')) return 'IsoDep';
      if (tag.techTypes.includes('android.nfc.tech.MifareClassic')) return 'Mifare Classic';
      if (tag.techTypes.includes('android.nfc.tech.MifareUltralight')) return 'Mifare Ultralight';
      return tag.techTypes[0] || 'unknown';
    }
