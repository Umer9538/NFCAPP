  ...
  const loadProfileData = async () => {
    try {
      ...
      if (profile) {
        ...
        setValue('gender', profile.gender as any || undefined);
        setValue('dateOfBirth', profile.dateOfBirth || '');
        setValue('height', profile.height?.toString() || '');
        setValue('weight', profile.weight?.toString() || '');
        setValue('bloodType', profile.bloodType as any || '');
        setValue('organDonor', profile.isOrganDonor || false);
  ...
  const onSubmit = async (data: EditProfileFormData) => {
    try {
      ...
      if (data.emergencyContact?.name) {
        const contacts = await contactsApi.getEmergencyContacts();
        const primaryContact = contacts.find(c => c.isPrimary);

        if (primaryContact) {
          await contactsApi.updateEmergencyContact(primaryContact.id, {
            name: data.emergencyContact.name,
            relationship: data.emergencyContact.relationship || '',
            phone: data.emergencyContact.phone || '',
          });
        } else {
          await contactsApi.addEmergencyContact({
            name: data.emergencyContact.name,
            relationship: data.emergencyContact.relationship || '',
            phone: data.emergencyContact.phone || '',
            isPrimary: true,
