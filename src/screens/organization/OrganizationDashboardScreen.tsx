 ...
 * Modern, professional corporate dashboard
 */
...
import type { OrganizationTabNavigationProp } from '@/navigation/types';
...
export default function OrganizationDashboardScreen() {
  const navigation = useNavigation<OrganizationTabNavigationProp>();
  const { user } = useAuth();
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    initialData: getMockOrgDashboardData(),
  });
  ...
  const organizationLabel = dashboardConfig.displayName || 'Corporate Account';
  ...
  return (
    ...
    >
      ...
      <LinearGradient
        colors={[PRIMARY[600], PRIMARY[500], PRIMARY[400]]}
        start={{ x: 0, y: 0 }}
        end={{ x: 1, y: 1 }}
          ...
          <View style={styles.profileSummary}>
            ...
            <View style={styles.profileDetails}>
              ...
              <View style={styles.accountBadge}>
                ...
                <Text style={styles.accountBadgeText}>{organizationLabel}</Text>
              </View>
            </View>
          </View>
        </View>
      ...
      <View style={styles.section}>
        <View style={styles.sectionHeader}>
          ...
          <Text style={styles.sectionBadge}>{reminders.filter(r => !r.completed).length}</Text>
        </View>
        ...
        {reminders.length === 0 ? (
          ...
          <View style={styles.remindersList}>
            {reminders.slice(0, 3).map((reminder: HealthReminder, index: number) => (
              <Pressable
                key={reminder.id}
                style={[
                  styles.reminderCard,
...
function getMockOrgDashboardData() {
  return {
    stats: {
      profileCompleteness: {
        percentage: 80,
    ...
    reminders: [
      ...
      },
      {
        ...
        description: 'Link your medical ID bracelet for emergency access',
